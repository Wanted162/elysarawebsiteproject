...
export function Chatbot() {
  ...
  const dragRef = useRef({ pointerId: -1, startX: 0, startY: 0, offsetX: 0, offsetY: 0, moved: false });
  ...
  const startDrag = (event: PointerEvent<HTMLButtonElement>) => {
    ...
    dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, offsetX: launcherOffset.x, offsetY: launcherOffset.y, moved: false };
    setDragging(true);
  };
  const moveDrag = (event: PointerEvent<HTMLButtonElement>) => {
    ...
    const x = Math.min(Math.max(drag.offsetX + deltaX, 8 - rect.left), window.innerWidth - 8 - rect.right);
    const y = Math.min(Math.max(drag.offsetY + deltaY, 8 - rect.top), window.innerHeight - 8 - rect.bottom);
